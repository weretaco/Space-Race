...
{
	private int actualX, actualY, assumedX, assumedY;
	...
	private void calculateAssumedDimensions() {
		...
		if(largestMultiple != 0) {
			if(largeDim == actualX) {
				assumedX = largestMultiple * NUMERATORS[assumedRatio];
				assumedY = largestMultiple * DENAMENATORS[assumedRatio];
			} else {
				assumedY = largestMultiple * NUMERATORS[assumedRatio];
				assumedX = largestMultiple * DENAMENATORS[assumedRatio];
			}
		}
	}
}
